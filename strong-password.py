...
def password_checker(passs):                                   #pass word strength checker
    ...
    password_check_und = re.compile(r'[a-z]')                  #checks if there is at least one under case letter.
    ...
    und_result = password_check_len.findall(passs)
    if len(len_result) >= 8 and len(cap_result) >= 1 and len(num_result) >= 1 and len(und_result) >= 1:              #does the check and prints result.
